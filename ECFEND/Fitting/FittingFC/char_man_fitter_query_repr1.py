...
class CharManFitterQueryRepr1(MultiLevelAttentionCompositeFitter):
    ...
    def _get_multiple_evidences_predictions_normal(self, query_ids: torch.Tensor,
        ...
        #     KeyWordSettings.FCClass.DocCharSource: e_chr_src_conts,
        #     KeyWordSettings.FIXED_NUM_EVIDENCES: n,
        #     KeyWordSettings.Query_Adj: query_adj                    # flatten->(n1 + n2 ..., R, R)
        # }
        #########################################

        # (B,)
        predictions = self._net(query_contents, evd_doc_contents, **additional_paramters)#原来
        # predictions, kl = self._net(query_contents, evd_doc_contents, **additional_paramters)#原来
        # predictions2, predictions, kl = self._net(query_contents, evd_doc_contents, **additional_paramters)#原来
        # predictions, consist = self._net(query_contents, evd_doc_contents, **additional_paramters)
        ...
        ##########################################################
        # return self._loss_func(predictions, labels.float()) + 0.4*consist
        # return self._loss_func(predictions, labels.float()) + 0.01*kl
        # return 0.5*self._loss_func(predictions2, labels.float()) + 0.5*self._loss_func(predictions, labels.float()) + 0.01*kl
    ...
    def evaluate(self, testRatings: interactions.ClassificationInteractions, K: int, output_ranking=False, **kargs):
        ...
        for query, evidences_info in testRatings.dict_claims_and_evidences_test.items():
            ...
            #                           **additional_information)  # shape = (len(labels), )
            if output_ranking:
                # probs, att_scores, _ = probs
                # probs, _, att_scores, _ = probs
    ...
    def load_best_model(self, val_interactions: interactions.ClassificationInteractions,
        ...
        total_num = sum(p.numel() for p in mymodel.parameters())
        trainable_num = sum(p.numel() for p in mymodel.parameters() if p.requires_grad)
        print("!!!!!!!Number2 of parameter: %.2fM" % (total_num/1e6))
        print("!!!!!!!Number2 of trainparameter: %.2fM" % (trainable_num/1e6))
    ...
    def load_best_model2(self, test_interactions: interactions.ClassificationInteractions, topN: int = 10):
        mymodel = self._net
        # print("Trained model: ", mymodel.out.weight)
        mymodel.load_state_dict(torch.load(self.saved_model))
        mymodel.train(False)
        ...
        print(auc_test)
        self.output_handler.save_error_analysis_testing(json.dumps(error_analysis_test, sort_keys=True, indent=2))
        self.output_handler.myprint('| Best Test F1_macro = %.5f | Best Test F1_micro = %.5f | Best Vad AUC = %.5f \n'
                                    '| Best Test Precision_True_class = %.5f | Best Test Recall_True_class = %.5f '
                                    '| Best Test F1_True_class = %.5f \n'
                                    '| Best Test Precision_False_class = %.5f | Best Test_Recall_False class = %.5f '
                                    '| Best Test F1_False_class = %.5f \n'
