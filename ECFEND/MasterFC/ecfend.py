...
def fit_models(args):
    ...
    secondary_log_folder = os.path.join(args.log, "log_results_%s_%s" % (args.dataset, args.iter_num))
    ...
    for i in range(args.num_folds):
        ...
        # b = valid_pack4_10.left["text_left"].str.lower().str.split().apply(len).max()
        ...
        print('parsing data')
        #######################################################
        # train_file = root + "/%sfold" % args.num_folds + "/train%s" % i + ".pkl"
        # valid_file = root + "/%sfold" % args.num_folds + "/valid%s" % i + ".pkl"
        # predict_file = root + "/predict%s" % i + ".pkl"
        # preprocessor_file = root + "/preprocessor%s" % i + ".pkl"
        # train_processed = None
        # valid_processed = None
        # predict_processed = None
        # if os.path.exists(train_file):
        #     t_file = open(train_file, "rb")
        #     train_processed = pickle.load(t_file)
        #     t_file.close()
        # else:
        #     train_processed = preprocessor.fit_transform(train_pack)  # This is a DataPack
        #     t_file = open(train_file, "wb")
        #     pickle.dump(train_processed, t_file)
        #     t_file.close()
        # if os.path.exists(valid_file):
        #     v_file = open(valid_file, "rb")
        #     valid_processed = pickle.load(v_file)
        #     v_file.close()
        # else:
        #     valid_processed = preprocessor.transform(valid_pack)
        #     v_file = open(valid_file, "wb")
        #     pickle.dump(valid_processed, v_file)
        #     v_file.close()
        # if os.path.exists(predict_file):
        #     p_file = open(predict_file, "rb")
        #     predict_processed = pickle.load(p_file)
        #     p_file.close()
        #     pr_file = open(preprocessor_file, "rb")
        #     preprocessor = pickle.load(pr_file)
        #     pr_file.close()
        # else:
        #     predict_processed = preprocessor.transform(predict_pack)
        #     p_file = open(predict_file, "wb")
        #     pickle.dump(predict_processed, p_file)
        #     p_file.close()
        #     pr_file = open(preprocessor_file, "wb")
        #     pickle.dump(preprocessor, pr_file)
        #     pr_file.close()
        #######################################################
        ...
        match_params["iter_num"] = args.iter_num
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type = str, default = 'Snopes', help = '[Snopes, Politifact, SnopesE]')
    ...
    #mec
    parser.add_argument('--iter_num', type=int, default=4, help="Iteration number in MEC")
