...
'text_right', 'length_right', 'extra_right', 'label']
        ...
        if mode == 'both':
            ...
            ################################################################
            # if summarize:
            #     self._apply_on_text_right2(func, rename, verbose=verbose)
            # else:
            #     self._apply_on_text_right(func, rename, verbose=verbose)
        else:
    ...
    ###########################################################################        
    # def _apply_on_text_right2(self, func, rename, verbose=1):
    #     name = rename or 'text_right'
    #     if verbose:
    #         tqdm.pandas(desc="Processing " + name + " with PegasusTokenizer")
    #         self._right[name] = self._right['text_right'].progress_apply(func, padding='longest', truncation=True, return_tensors="pt")
    #     else:
    #         self._right[name] = self._right['text_right'].apply(func, padding='longest', truncation=True, return_tensors="pt")
    ########################################################################################################################
