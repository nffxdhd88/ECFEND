...
# import torch
# def tokenizing(input, padding='longest', truncation=True, return_tensors="pt", **kwargs):
#     src_text = ' '.join(input)
...
class CharManPreprocessor(BasePreprocessor):
    ...
    def transform(self, data_pack: DataPack, verbose: int = 1) -> DataPack:
        ...
        # data_pack.apply_on_text(self._context['vocab_unit'].tokenizing,
        #                         mode='right', inplace=True, verbose=verbose)
        ####################################################################################
